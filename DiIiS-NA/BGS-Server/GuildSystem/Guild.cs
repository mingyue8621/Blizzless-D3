...
namespace DiIiS_NA.LoginServer.GuildSystem
{
	public class Guild
	{
		...
		public void AddMember(GameAccount account)
		{
			...
			{
				Logger.Warn("当帐户 {0} 已经是公会成员时，尝试将其添加到公会", account.Owner.BattleTag);
				return;
			}

			if (this.IsClan && account.Clan != null)
			{
				Logger.Warn("当帐户 {0} 已经是另一个氏族的成员时，尝试将其添加到氏族", account.Owner.BattleTag);
				return;
			}
		...
		public void RemoveMember(GameAccount account)
		{
			...
			if (!HasMember(account))
			{
				Logger.Warn("试图从公会 {1}#{2} 中删除非会员帐户 {0}", account.Owner.BattleTag, this.Name, this.PersistentId);
				return;
			}
			else if (account.Clan != this && !account.Communities.Contains(this))
			{
				Logger.Warn("客户 {0} 已从与他无关的公会 ({1}#{2}) 中移除.", account.Owner.BattleTag, this.Name, this.PersistentId);
			}
		...
		public uint GetRank(ulong GameAccountId)
		{
			...
			{
				Logger.Warn("客户 {0} 在与他没有关联的公会 ({1}#{2}) 上调用了 GetRank().", GameAccountId, this.Name, this.PersistentId);
				return 0;
			}
		}
		...
		public string GetMemberNote(ulong GameAccountId)
		{
			...
			{
				Logger.Warn("客户 {0} 在与他没有关联的公会 ({1}#{2}) 上调用了 GetMemberNote().", GameAccountId, this.Name, this.PersistentId);
				return "";
			}
		}
