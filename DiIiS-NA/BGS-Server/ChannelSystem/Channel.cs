	...
	public class Channel : RPCObject
	{
		...
		public Channel(BattleClient client, bool isGameChannel = false, ulong remoteObjectId = 0, bool isChatChannel = false)
		{
			...
			if (this.IsChatChannel)
				...
				{
					if (this == null || this._dissolved) throw new Exception("频道为空");
		...
		public void AddMember(BattleClient client)
		{
			if (HasUser(client))
			{
				Logger.Warn("尝试将客户端 {0} 添加到通道时，它已经是通道的成员", client.SocketConnection.RemoteAddress.ToString());
				return;
			}
		...
		public void RemoveMember(BattleClient client, RemoveReason reason, bool wentOffline = false)
		{
			...
			{
				if (client != null)
					Logger.Warn("无法从频道 {1} 中删除客户端 {0}.", client.SocketConnection.RemoteAddress.ToString(), this.ToString());
				return;
			}
			else if (!HasUser(client))
			{
				Logger.Warn("试图从频道 {1} 中删除非成员客户端 {0}.", client.SocketConnection.RemoteAddress.ToString(), this.ToString());
				return;
			}
			else if (!client.Channels.ContainsValue(this) && !client.ChatChannels.Contains(this))
			{
				Logger.Warn("客户 {0} 已从与其没有关联的频道 ({1}) 中删除.", client.SocketConnection.RemoteAddress.ToString(), this.ToString());
			}
			...
			lock (this.Members)
			{
				...
				if (this.IsGameChannel)
				{
					...
					Logger.Warn("客户 {0} 离开游戏频道 {1}.", client, this);
				}
				else if (this.IsChatChannel)
				{
					client.ChatChannels.Remove(this);
					Logger.Warn("客户 {0} 离开聊天频道 {1}.", client, this);
				}
				else
				{
					client.PartyChannel = null;
					Logger.Warn("客户 {0} 左方频道 {1}.", client, this);
				}
			}
		}
		...
		public void SendMessage(BattleClient client, bgs.protocol.channel.v1.Message message)
		{
			...
			{
				client.SendServerWhisper(string.Format("管理员已将您在公共聊天频道中静音。\n剩余时间：{0} 秒.", client.Account.MuteTime - DateTime.Now.ToUnixTime()));
				return;
			}
		...
		{
			return String.Format("{{ 频道: [id: {0}] [所有者: {1}] }}", this.DynamicId, this.Owner != null ? this.Owner.Account.GameAccount.CurrentToon.ToString() : "N/A");
		}
	}
}
