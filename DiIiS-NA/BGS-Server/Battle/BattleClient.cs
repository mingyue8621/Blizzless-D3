...
namespace DiIiS_NA.LoginServer.Battle
{
	...
	{
		private static readonly Logger Logger = LogManager.CreateLogger("F - 客户端");
		...
		public BattleClient(ISocketChannel socketChannel, DotNetty.Handlers.Tls.TlsHandler TLS)
		{
			...
			if (SocketConnection.Active)
				Logger.Trace("客户端 - {0} - 成功加密连接", socketChannel.RemoteAddress);
		}
			...
			if (msg.GetHeader().ServiceId == RESPONSE_SERVICE_ID)
			{
				...
				if (done != null)
				{
					...
					if (service != null)
					{
						...
						Logger.Debug(String.Format("传入响应：无法识别服务(id: %d, hash: 0x%04X)", header.ServiceId, header.ServiceHash));
				}
			}
			else
			{
				...
				if (service != null)
				{
					...
					*/
...
#if DEBUG
							Logger.Warn("调用：{0}，服务散列：{1}，方法: {2}, ID: {3}", service.GetType().Name, header.ServiceHash, method.Name, header.MethodId);
#endif
					...
					{
						Logger.Warn("未实施的服务方法: {0}.{1}", service.GetType().Name, method.Name);
					}
				}
				else
					Logger.Warn(String.Format("客户端访问了一个未连接的服务(id: {0}, hash: {1}  Method id: {2})", header.ServiceId, header.ServiceHash, header.MethodId));
		...
		public void CallMethod(MethodDescriptor method, IRpcController controller, IMessage request, IMessage responsePrototype, Action<IMessage> done)
		{
			...
			{
				Logger.Warn("找不到绑定到客户端的服务 {0} [0x{1}].", serviceName, serviceHash.ToString("X8"));
				return;
			}
		...
		public void SendMOTD()
		{
			...
			var motd = "欢迎来到暗黑破坏神3 Alpha 版服务器!";
