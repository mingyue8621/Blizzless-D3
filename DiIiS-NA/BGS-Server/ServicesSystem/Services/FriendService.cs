...
namespace DiIiS_NA.LoginServer.ServicesSystem.Services
{
    ...
    public class FriendService : FriendsService, IServerService
	{
		...
		{
			Logger.Trace("订阅() {0}", ((controller as HandlerController).Client));
		...
		public override void SendInvitation(IRpcController controller, SendInvitationRequest request, Action<NoData> done)
		{
			...
			Logger.Trace("加好友请求正文: {0}", friendRequest.ToString());
			...
			Logger.Trace("{0} 发送了 {1} 个好友邀请.", ((controller as HandlerController).Client).Account, invitee);
		...
		{
			Logger.Trace("{0} 接受了朋友邀请.", ((controller as HandlerController).Client).Account);
		...
		{
			Logger.Trace("{0} 拒绝朋友邀请.", ((controller as HandlerController).Client).Account);
		...
		{
			Logger.Trace("{0} 删除了 ID 为 {1} 的朋友.", ((controller as HandlerController).Client).Account, request.TargetId);
		...
		{
			Logger.Trace("查看朋友(): {0}.", request.ToString());
		...
		{
			Logger.Trace("更新朋友状态(): {0}.", request.ToString());
