	...
	public static class AccountManager
	{
		...
		public static Account GetAccountByEmail(string email)
		{
			...
			else
			{
				...
				if (dbAcc.Count() == 0)
				{
					Logger.Warn("用电子邮件获取帐户 {0}: DBAccount is null!", email);
					return null;
				}
				if (dbAcc.First() == null)
				{
					Logger.Warn("用电子邮件获取帐户 {0}: DBAccount id is null!", email);
					return null;
				}
				else
					Logger.Debug("用电子邮件获取帐户 id -  \"{0}\"", dbAcc.First().Id);
		...
		public static Account GetAccountByBattletag(string battletag)
		{
			...
			if (dbAcc.Count() == 0)
			{
				Logger.Warn("通过战网获取帐户 {0}: DBAccount is null!", battletag);
				return null;
			}
			//else
		...
		public static Account GetAccountByName(string btname) //pretty bad to use it
		{
			...
			if (dbAcc.Count() == 0)
			{
				Logger.Warn("按名称获取帐户 {0}: DBAccount is null!", btname);
				return null;
			}
			return GetAccountByDBAccount(dbAcc.First());
		}
