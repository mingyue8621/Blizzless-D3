...
namespace DiIiS_NA.REST
{
    public class RestSession : SocketBase
    {
        ...
        {
            SendResponseHtml(HttpCode.OK, "欢迎来到暴雪战网" + 
                                          "\nBuild " + Program.Build +
                                          "\nSupport: 2.7.4");
        }
        ...
        public void HandleLoginRequest(HttpHeader request)
        {
            ...
            if (loginForm == null)
            {
                ...
                loginResult.ErrorMessage = "连接战网时发生内部错误。 请稍后重试.";
            ...
            if (result)
            {
                ...
                Logger.Warn("验证完成：登录 - {0}。 REST 连接已终止.", login);
            }
            else
            {
                loginResult.AuthenticationState = "LOGIN";
                loginResult.ErrorCode = "UNABLE_TO_DECODE";
                loginResult.ErrorMessage = "HEBEPHO! WRONG!错误！ 你会说英语吗？!";
                SendResponse(HttpCode.BadRequest, loginResult);
                Logger.Error("身份验证失败：登录 - {0}。 REST 连接已终止.", login);
            }
            CloseSocket();
