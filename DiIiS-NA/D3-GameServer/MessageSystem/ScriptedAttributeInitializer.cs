				...
				script = Regex.Replace(script, @"([A-Za-z_]\w*)(\.Agg)?(\#[A-Za-z_]\w*)?(?=[^\(\w]|\z)( \?)?",
					(match) =>
					{
						...
						if (!attributeLookup.ContainsKey(identifierName))
							throw new ScriptedAttributeInitializerError("解析的标识符无效: " + identifierName);
						...
						if (match.Groups[3].Success)
						{
							switch (match.Groups[3].Value.ToUpper())
							{
								...
								default:
									throw new ScriptedAttributeInitializerError("错误处理属性脚本，标识符中的键无效: " + match.Groups[3].Value);
							}
