...
namespace DiIiS_NA.GameServer.ClientSystem.Base
{
	public class Server
	{
		...
		public virtual bool Listen(string bindIP, int port)
		{
			...
			if (_disposed) throw new ObjectDisposedException(this.GetType().Name, "服务器已被处理.");

			// Check if the server is already listening.
			if (IsListening) throw new InvalidOperationException("服务器已经在监听.");
			this.Port = port;

			Acceptor = new AsyncAcceptor();
			if (!Acceptor.Start(bindIP, port))
			{
				Logger.Fatal("监听启动 AsyncAcceptor 失败 {0}", bindIP);
				return false;
			}
