    ...
    public class ClientManager : IMessageConsumer
    {
		...
		public void OnJoinGame(GameClient client, JoinBNetGameMessage message)
		{
			...
			if (game == null)
			{
				...
				{
					Logger.Warn("客户端提供的消息。游戏 ID 不存在，正在退出客户端...");
			...
			lock (game)
			{
				...
				Logger.Debug("玩家 {0}[PlayerIndex: {1}, PlayerGroupIndex: {2}] 已连接.", client.Player.Toon.Name, client.Player.PlayerIndex, client.Player.PlayerGroupIndex);
				...
				Logger.Debug("登录时间:" + toon.LoginTime.ToString());
